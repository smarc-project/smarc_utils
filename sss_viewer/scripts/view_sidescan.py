...
import rospy
import cv2
from smarc_msgs.msg import SidescanPositioned
...
def callback(img, msg):
    ...
    port = np.array(bytearray(msg.sidescan.sidescan.port_channel), dtype=np.ubyte)
    stbd = np.array(bytearray(msg.sidescan.sidescan.starboard_channel), dtype=np.ubyte)
...
rospy.Subscriber("/sam/payload/sidescan", SidescanPositioned, partial(callback, img))
