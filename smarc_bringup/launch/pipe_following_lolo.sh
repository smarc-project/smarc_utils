...
tmux new-window -t $SESSION:3 -n 'large_smarc_auv'
...
tmux send-keys "roslaunch smarc_auvs upload_large_smarc_auv.launch x:=244.3 y:=-23.4 z:=-93.0 yaw:=3.0"

tmux select-window -t $SESSION:4
tmux send-keys "rosrun smarc_keyboard_teleop large_auv_teleop.py"
